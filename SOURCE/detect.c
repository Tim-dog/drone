...
void detect_screen(int record[21][26], char *nowfield, int language)
{
    ...
    // 清除鼠标指针在屏幕上的痕迹，MouseX和MouseY是鼠标当前坐标
    clrmous(MouseX, MouseY);

    // 绘制主显示区域，根据record数组、当前场景名nowfield和语言language来决定内容
    paint_field(record, nowfield, language);

    // 在屏幕上绘制一个日历，通常用于显示当前日期
    put_calender();

    // 绘制模式选择按钮，PAINT模式表示要在屏幕上画出来
    mode_button(PAINT);

    // 绘制“开始”按钮，PAINT模式表示绘制按钮而不是响应点击
    start_button(PAINT);

    // 绘制“暂停”按钮，PAINT模式表示绘制按钮而不是响应点击
    pause_button(PAINT);

    // 绘制“图表”按钮，PAINT模式表示绘制按钮而不是响应点击
    chart_button(PAINT);
}
...
int detect_page(char *username ,char *nowfield,int language)
{
    ...
    while(1)
    {
        ...
        else if( mouse_press(15,20,90,60)==1 ) //�������ֵ��
        {
            strcpy(compare , date );
            setfillstyle(SOLID_FILL,WHITE);
            bar(11,25,90,69);
            ...
            if(strcmp(compare , date )!= 0  ) {
                ...
                grow(record , atoi(date));//ÿ�����ڸı�ʱ,��ˢ���Ҳ��ͼ
                field_flag = 1 ;
            }
        }
        else if( mouse_press(68,73,92,95)==1 )// �ϼ�ͷ
        {
            strcpy(compare , date );
            setfillstyle(SOLID_FILL,WHITE);
            bar(11,25,90,69);
        ...
        else if( mouse_press(68,95,92,117)==1 )// �¼�ͷ
        {
            strcpy(compare , date );
            setfillstyle(SOLID_FILL,WHITE);
            bar(11,25,90,69);
        ...
        else if( mouse_press(5,330,95,369)==1  && presentmode[0]=='h')//route���
        {
            ...
            if(handmode_flag == 1 ) //����hand���ĵ�ͼ ��յ�ͼ
            {
                paint_field_right(record , nowfield,language);
        ...
        if(mode == 1) //ѡ��hand��ѡ��
        {
            ...
            {
                newmouse(&MouseX,&MouseY,&press);
                if( mouse_press(110,50,630,470)==1 )//���ڻ�ͼ�����ҵ��
                {
                    if(k==0) {
                        i = (470-MouseY)/20;
                        j = (MouseX - 110)/20;
        ...
        if(timecount%600000 == 0) 
        {
            ...
            if(date % 3 == 0) //�������
            {
                ...
                if(automode == 1 ) {
                    ...
                    for(i=0;i<k;i++)
                    {
                        if( strlen(dronerecord[house[i].drone_index].name)==0  || strcmp(dronerecord[house[i].drone_index].weather , weather )==0 ) //���ܷɻ��߸÷���û�����˻�
                        {
